...
resource "aws_ecr_repository" "my_repo" {
  ...
  tags = {
    repository = "https://github.com/EverydayMoney/EverydayMoney-API"
  }
}
  ...
  [
    {
      ...
      "environment": [
          ...
          },
          {
              ...
              "value": "${var.AWS_PUBLIC_BUCKET_NAME}"
          },
      ],
    }
  ])
...
resource "aws_ecs_service" "my_service" {
  ...
  load_balancer {
    ...
    container_port   = 4700 # Specifying the container port
  }
...
#   name        = "${var.app}-${var.environment}-allow_tls"
...
resource "aws_security_group" "my_lb_security_group" {
  name            = "${var.app}-${var.environment}-lb-sg"
  ingress {
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
